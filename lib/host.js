...
// 选出会话目录里最高世代的 zstd 日志；没有返回 null。
function pickSessionLog(sessionPath) {
  let best = null;
  let bestRank = -1;
  let files = [];
  try { files = readdirSync(sessionPath); } catch (e) { return null; }
  for (const f of files) {
    const lower = f.toLowerCase();
    if (!lower.endsWith('.zstd') && !lower.endsWith('.zst')) continue;
    const rank = zstdGenerationRank(f);
    if (rank > bestRank) { bestRank = rank; best = f; }
  }
  return best ? join(sessionPath, best) : null;
}

// 扫描本机会话根目录，找出某会话 id 已落位的所有位置（工作区文件夹名 + 目录）。
// DSH 按会话 id 全局索引；同一 id 出现两份会让头部解析选错，导致 attach 拒绝。
function findSessionHomesOnDisk(sessionsRoot, sessionId) {
  const homes = [];
  let folders = [];
  try { folders = readdirSync(sessionsRoot); } catch (e) { return homes; }
  for (const folder of folders) {
    const dir = join(sessionsRoot, folder, sessionId);
    try { if (statSync(dir).isDirectory()) homes.push({ folder, dir }); } catch (e) { /* skip */ }
  }
  return homes;
}

// 读取会话日志头（第 0 帧 = 恰好一行会话头 JSON）：内嵌 id/createdAt/cwd，
// 是会话身份的权威来源。工作区注册表/manifest 缺失时（如只导了会话的归档、
// 旧版导出），用它回退解析源工作区路径，映射 UI 与 doImport 都以此为键。
function readSessionLogHeader(logPath) {
  if (!logPath) return null;
  try {
    const data = readFileSync(logPath);
    const { frames } = scanZstdFrames(data, 1);
    if (frames.length === 0) return null;
    const header = JSON.parse(Buffer.from(zstdDecompressSync(data.subarray(frames[0].start, frames[0].end))).toString('utf-8').trim());
...
function apply(ctx) {
  ...
  function extractSessionTitle(sessionPath) {
    try {
      const logPath = pickSessionLog(sessionPath);
      if (!logPath) return null;
  ...
  async function doExportAsync(outputDir, options) {
    ...
    // 工作区注册表是导入映射的关键元数据（源路径 → 本机目标）。只导会话、
    // 不选配置文件时也必须携带，否则跨机器导入既不显示映射 UI，也无法落位。
    if (opts.sessions) {
      try {
        const regSrc = join(dshHome, 'storages', 'workspace.json');
        if (existsSync(regSrc)) {
          mkdirSync(join(staging, 'configs'), { recursive: true });
          writeFileSync(join(staging, 'configs', 'storages_workspace.json'), readFileSync(regSrc));
        }
      } catch (e) { /* skip */ }
    }
  ...
  async function doImport(importPath, selection) {
    ...
    try {
      ...
      // 同 id 会话在别的本机工作区已有落位时，本次导入按"移动"处理
      const movedSessionIds = [];   // 已完成移动清理（旧目录已删）的会话
      const movedFromFolders = [];  // 被移除拷贝所在的旧工作区文件夹名
      const movedConflicts = [];    // 旧位置会话正被打开，不能移动
      const movedTargets = new Map(); // sid -> 移动后合法所在的工作区文件夹名
      ...
      if (existsSync(sessionsDir)) {
        ...
        const toRawPath = (v) => {
          ...
          // 注册表缺失时 manifest 里的 workspace 常是编码目录名（--...--），
          // 不是可用路径：必须返回 null，让日志头 cwd 兜底接管；只有形如
          // 绝对路径（POSIX / Windows 盘符 / UNC）的值才原样放行。
          const s = String(v);
        ...
        const resolveRawWorkspace = (folderName, sessionId) => {
          ...
          const raw = fromManifest ? toRawPath(fromManifest) : null;
          if (raw) return raw;
        ...
        const importSessionDir = (sessPath, id, rawWorkspace) => {
          ...
          // 同 id 在其他本机工作区已有落位或登记 → 按"移动"处理：删旧位置
          // （DSH 按会话 id 全局索引，两份同 id 拷贝会让头部解析选错、attach 被
          // 拒）、解除旧登记、丢弃投影缓存文档让标题按新身份重建。会话正被
          // 打开时不移动，如实报告冲突。
          const otherHomes = findSessionHomesOnDisk(join(dshHome, 'sessions'), id)
            .filter((h) => h.folder !== workspaceFolder);
          // 注册表残留：旧拷贝可能已不在磁盘，但旧工作区记录仍登记着该 id
          const foreignAccounted = new Set();
          try {
            const regRaw = JSON.parse(readFileSync(join(dshHome, 'storages', 'workspace.json'), 'utf-8'));
            for (const [, rec] of Object.entries((regRaw.tables && regRaw.tables.workspaces) || {})) {
              if (!rec || !rec.path || !Array.isArray(rec.sessionIds) || !rec.sessionIds.includes(id)) continue;
              const folder = encodeWorkspaceFolder(rec.path);
              if (folder !== workspaceFolder) foreignAccounted.add(folder);
            }
          } catch (e) { /* skip */ }
          if (otherHomes.length > 0 || foreignAccounted.size > 0) {
            const sessionsSvc = ctx.get('sessions');
            const live = sessionsSvc && typeof sessionsSvc.get === 'function' ? sessionsSvc.get(id) : undefined;
            if (live !== undefined) {
              // 活会话：旧位置在写入中，动它或再写一份都会造成同 id 双拷贝
              // （DSH 启动扫描遇重复 id 会拒绝加载工作区服务）。整体跳过。
              movedConflicts.push({ id, from: otherHomes.map((h) => h.folder) });
              return { wrote: false, skipped: true, liveConflict: true };
            }
            for (const h of otherHomes) {
              try { rmSync(h.dir, { recursive: true, force: true }); } catch (e) { /* skip */ }
              movedFromFolders.push(h.folder);
            }
            for (const folder of foreignAccounted) movedFromFolders.push(folder);
            movedSessionIds.push(id);
            movedTargets.set(id, workspaceFolder);
            try { rmSync(join(dshHome, 'storages', 'session_projcache', 'sessions', id + '.json'), { force: true }); } catch (e) { /* skip */ }
          }
          ...
          if (wrote > 0) {
            if (zstdLogName) {
              // 从目标目录实际内容取最高世代日志（移动场景下旧文档已删，需重建）
              const dstLog = pickSessionLog(dstSessPath);
              if (dstLog) projcacheCandidates.push({ id, logPath: dstLog, cwd: mapped });
            }
        ...
        for (const sd of readdirSync(sessionsDir)) {
          ...
          if (subDirs.length > 0 && wsById[sd] === undefined) {
            ...
            for (const id of subDirs) {
              ...
              try {
                let rawWs = resolveRawWorkspace(sd, id);
                if (!rawWs) {
                  // 注册表/manifest 缺失（只导会话的归档）：日志头内嵌 cwd
                  // 是权威源路径，与映射 UI 暴露的键一致。
                  const header = readSessionLogHeader(pickSessionLog(join(sessPath, id)));
                  rawWs = header && typeof header.cwd === 'string' ? header.cwd : null;
                }
                const result = importSessionDir(join(sessPath, id), id, rawWs);
                if (result.wrote) {
                  restoredAny = true;
            ...
            try {
              let rawWs = resolveRawWorkspace(sd, sd);
              if (!rawWs) {
                const header = readSessionLogHeader(pickSessionLog(sessPath));
                rawWs = header && typeof header.cwd === 'string' ? header.cwd : null;
              }
              const result = importSessionDir(sessPath, sd, rawWs);
      ...
      // 移动导入收尾：从旧工作区解除登记（活体 best-effort；注册表文件仅用于
      // 文件夹名 → 路径映射，真正的解除走运行中的 registry 实体）。
      if (movedSessionIds.length > 0) {
        ...
        if (registry !== undefined) {
          ...
          try {
            const regRaw = JSON.parse(readFileSync(join(dshHome, 'storages', 'workspace.json'), 'utf-8'));
            for (const [, rec] of Object.entries((regRaw.tables && regRaw.tables.workspaces) || {})) {
              if (rec && rec.path) folderToPath.set(encodeWorkspaceFolder(rec.path), rec.path);
            }
          } catch (e) { /* skip */ }
          for (const folder of movedFromFolders) {
            const oldPath = folderToPath.get(folder);
            if (!oldPath) continue;
            try {
              const oldEntity = await registry.resolveByPath(oldPath);
              if (oldEntity && typeof oldEntity.detachSession === 'function') {
                for (const sid of movedSessionIds) await oldEntity.detachSession(sid);
              }
            } catch (e) { /* best-effort */ }
          }
        }
        restored.movedSessions = movedSessionIds.length;
      }

      // 收尾清扫：移动过程中旧位置可能被活会话/写入缓冲重新写回（DSH 启动
      // 扫描遇同 id 双拷贝会拒绝加载工作区服务），退出前再扫一遍。
      if (movedSessionIds.length > 0) {
        let reswept = 0;
        for (const sid of movedSessionIds) {
          const legal = movedTargets.get(sid);
          for (const home of findSessionHomesOnDisk(join(dshHome, 'sessions'), sid)) {
            if (home.folder === legal) continue;
            try { rmSync(home.dir, { recursive: true, force: true }); reswept++; } catch (e) { /* skip */ }
          }
        }
        if (reswept > 0) restored.resweptStrays = reswept;
      }
      if (movedConflicts.length > 0) skipped.liveSessionConflicts = movedConflicts.length;

      // 导入的会话可能带着源机器的归档状态（registry-global archivedSessionIds），
      // 归档会话在任何工作区列表都不显示。registry 服务没有移出归档的 API，
      // 按既有做法改注册表文件（重启后生效）。
      if (restoredByTarget.size > 0) {
        try {
          const regPath = join(dshHome, 'storages', 'workspace.json');
          if (existsSync(regPath)) {
            const regRaw = JSON.parse(readFileSync(regPath, 'utf-8'));
            const g = regRaw.global || (regRaw.global = {});
            if (!Array.isArray(g.archivedSessionIds)) g.archivedSessionIds = [];
            const before = g.archivedSessionIds.length;
            for (const [, info] of restoredByTarget.entries()) {
              for (const sid of info.sessionIds) {
                g.archivedSessionIds = g.archivedSessionIds.filter((s) => s !== sid);
              }
            }
            const removed = before - g.archivedSessionIds.length;
            if (removed > 0) {
              writeFileSync(regPath, JSON.stringify(regRaw, null, 2), 'utf-8');
              restored.unarchivedSessions = (restored.unarchivedSessions || 0) + removed;
            }
          }
        } catch (e) { /* skip */ }
      }
          ...
          if (subDirs.length > 0 && wsById[sd] === undefined) {
            ...
            for (const id of subDirs) {
              ...
              try {
                ...
                // 日志头内嵌 cwd = 源工作区的权威路径（注册表缺失时的映射依据）
                const header = readSessionLogHeader(pickSessionLog(sp));
                const sourcePath = header && typeof header.cwd === 'string' ? header.cwd : null;
                result.sessions.push({ id, title: title || id.slice(-8), workspace: sd, fileCount: files.length, totalSize, sourcePath });
              } catch (e) { /* skip */ }
            }
          } else {
            // Legacy layout
            ...
            try {
              ...
              const header = readSessionLogHeader(pickSessionLog(sessPath));
              const sourcePath = header && typeof header.cwd === 'string' ? header.cwd : null;
              result.sessions.push({ id: sd, title: title || sd.slice(-8), workspace, fileCount: files.length, totalSize, sourcePath });
            } catch (e) { /* skip */ }
          }
        }

        // 注册表缺失/不全时，用日志头内嵌 cwd 兜底暴露源工作区路径——
        // 没有它，映射 UI 不会渲染，跨机器导入的会话也无法映射落位。
        const seenWsPaths = new Set(result.sourceWorkspaces.map(ws => ws.path));
        for (const s of result.sessions) {
          if (!s.sourcePath || seenWsPaths.has(s.sourcePath)) continue;
          seenWsPaths.add(s.sourcePath);
          result.sourceWorkspaces.push({ path: s.sourcePath, title: '', exists: existsSync(s.sourcePath) });
        }
      }
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        const body = JSON.parse(await readBody(req));
        // selection（含工作区映射）必须透传给 doImport，否则手动路径导入的
        // 映射会被静默丢弃
        const result = await doImport(body.importPath, body.selection || null);
        res.writeHead(200, { 'content-type': 'application/json' });
        res.end(JSON.stringify(result));
      } catch (e) {
        res.writeHead(500, { 'content-type': 'application/json' });
