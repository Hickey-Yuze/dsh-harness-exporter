...
function waitZstdReady(maxMs) {
  const deadline = Date.now() + (maxMs || 5000);
  while (!zstdReady && Date.now() < deadline) {
    // busy-wait briefly; init normally completes in well under a second
...
// DSH workspace folder encoding, verified against real on-disk folders:
//   /Users/yuze/.../Yuze-音乐网站 -> --Users-yuze-...-Yuze-~97F3~4E50~7F51~7AD9--
// Slashes (and Windows backslashes) become '-', ':' is dropped, every other
// non-safe character becomes '~' + 4-digit uppercase hex codepoint.
function encodeWorkspaceFolder(p) {
  ...
  for (const ch of body) {
    ...
    const safe = (code >= 0x61 && code <= 0x7A) || (code >= 0x41 && code <= 0x5A) ||
      (code >= 0x30 && code <= 0x39) || ch === '-' || ch === '_' || ch === '.';
    out += safe ? ch : '~' + code.toString(16).toUpperCase().padStart(4, '0');
  }
  return '--' + out + '--';
}
...
function rewriteZstdIdentity(data, fromPath, toPath) {
  if (!waitZstdReady()) return null;
  try {
    const streaming = new zstdInstance.Streaming();
    const decompressed = streaming.decompress(new Uint8Array(data));
    let text = Buffer.from(decompressed).toString('utf-8');
    const escFrom = JSON.stringify(String(fromPath)).slice(1, -1);
    const escTo = JSON.stringify(String(toPath)).slice(1, -1);
    if (!text.includes(escFrom) && !text.includes(String(fromPath))) return data;
    text = text.split(escFrom).join(escTo).split(String(fromPath)).join(String(toPath));
    const simple = new zstdInstance.Simple();
    const compressed = simple.compress(new Uint8Array(Buffer.from(text, 'utf-8')), 3);
          ...
          if (subDirs.length > 0 && wsById[sd] === undefined) {
            ...
            for (const id of subDirs) {
              ...
              try {
                ...
                if (result.wrote) restoredAny = true;
              } catch (e) { /* skip */ }
            }
