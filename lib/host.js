  ...
  async function doImport(importPath, selection) {
    ...
    try {
      ...
      if (existsSync(sessionsDir)) {
        ...
        // Helper to resolve workspace path with mapping support
        const resolveWorkspacePath = (sourceWorkspaceFolder) => {
          // sourceWorkspaceFolder is the directory name under sessions/
          // It's an encoded path like --C-Users-~534E~7855-Desktop-Yuze-2026-DSH--
          // We need to find the actual source path from the registry and apply mapping
          
          if (sourceRegistryForAttach) {
            const srcWorkspaces = (sourceRegistryForAttach.tables && sourceRegistryForAttach.tables.workspaces) || {};
            
            // First, try exact match with encoded path
            for (const [id, rec] of Object.entries(srcWorkspaces)) {
              if (!rec || !rec.path) continue;
              
              // Encode the workspace path the same way DSH does
              const cleanPath = rec.path.replace(/^--+|--+$/g, '');
              const expectedFolder = '--' + cleanPath.replace(/^\/+|\/+$/g, '').replace(/[\/\\]/g, '-') + '--';
              
              if (expectedFolder === sourceWorkspaceFolder) {
                if (workspaceMapping[rec.path]) {
                  return workspaceMapping[rec.path];
                }
                return rec.path;
              }
            }
            ...
            // Windows may encode non-ASCII characters differently (e.g., 华硕 -> ~534E~7855)
            // So we check if the folder name contains key parts of the source path
            for (const [id, rec] of Object.entries(srcWorkspaces)) {
              if (!rec || !rec.path) continue;
              
              // Extract key parts from the source path (last 2-3 segments)
              const pathParts = rec.path.split(/[\/\\]/).filter(p => p.length > 0);
              const keyParts = pathParts.slice(-3).join('-').toLowerCase();
              
              // Check if the folder name contains these key parts
              if (sourceWorkspaceFolder.toLowerCase().includes(keyParts.toLowerCase())) {
                if (workspaceMapping[rec.path]) {
                  return workspaceMapping[rec.path];
                }
                return rec.path;
          ...
          // No mapping found, return the original folder name
          return sourceWorkspaceFolder;
        };
        const importSessionDir = (sessPath, id, workspace) => {
          // Resolve the actual workspace path (apply mapping if needed)
          const resolvedPath = resolveWorkspacePath(workspace);
          
          // Check if the resolved path exists on this machine
          // If not, skip importing this session
          if (!existsSync(resolvedPath)) {
            skipped.missingWorkspaceSessions = (skipped.missingWorkspaceSessions || 0) + 1;
            return { wrote: false, resolvedPath, workspaceFolder: null, skipped: true };
          }
          
          // Create a folder name from the resolved path for storage
          // DSH expects format: --<encoded-path>--
          // First, remove any existing -- prefix/suffix to avoid duplication
          const cleanPath = resolvedPath.replace(/^--+|--+$/g, '');
          // Remove leading/trailing slashes, replace remaining with -, add -- prefix/suffix
          const workspaceFolder = '--' + cleanPath.replace(/^\/+|\/+$/g, '').replace(/[\/\\]/g, '-') + '--';
          ...
          return { wrote: wrote > 0, resolvedPath, workspaceFolder, skipped: false };
        };
        for (const sd of readdirSync(sessionsDir)) {
          ...
          if (subDirs.length > 0 && wsById[sd] === undefined) {
            ...
            for (const id of subDirs) {
              ...
              try {
                const result = importSessionDir(join(sessPath, id), id, sd);
                if (result.skipped) continue; // Skip if workspace path doesn't exist
                if (result.wrote) {
                  restoredAny = true;
            ...
            if (restoredAny) {
              restored.sessions++;
              // Track mapped workspaces for later registration
              if (!restored.workspaceMappings) restored.workspaceMappings = [];
              restored.workspaceMappings.push({ sourceFolder: sd, resolvedPath, workspaceFolder });
            }
          } else {
            ...
            try {
              const result = importSessionDir(sessPath, sd, workspace);
              if (result.skipped) continue; // Skip if workspace path doesn't exist
              if (result.wrote) {
                restored.sessions++;
                if (!restored.workspaceMappings) restored.workspaceMappings = [];
                restored.workspaceMappings.push({ sourceFolder: workspace, resolvedPath: result.resolvedPath, workspaceFolder: result.workspaceFolder });
              }
            } catch (e) { /* skip */ }
          }
        }
      ...
      if (sourceRegistryForAttach) {
        ...
        if (registry !== undefined) {
          ...
          const attachErrors = [];
          for (const [wsId, rec] of Object.entries(srcWorkspaces)) {
            if (!rec || !rec.path || !Array.isArray(rec.sessionIds) || rec.sessionIds.length === 0) continue;
            // Use mapped path if available, otherwise use original path
            const targetPath = workspaceMapping[rec.path] || rec.path;
            if (!existsSync(targetPath)) continue; // missing folder → cannot claim its sessions
            let entity = null;
            try {
              entity = await registry.create(targetPath, rec.title);
              attachedWorkspaces++;
            } catch (e) {
              attachErrors.push(`create(${targetPath}): ` + String((e && e.message) || e).slice(0, 140));
              continue;
            }
