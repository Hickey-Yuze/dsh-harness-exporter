      ...
      if (existsSync(sessionsDir)) {
        ...
        const resolveWorkspacePath = (sourceWorkspaceFolder) => {
          ...
          if (sourceRegistryForAttach) {
            ...
            // Decode the folder name back to a path for comparison
            // Remove -- prefix/suffix, replace - with /
            const decodedFolder = sourceWorkspaceFolder
              .replace(/^--+|--+$/g, '')
              .replace(/-/g, '/');
            
            // Find workspace by comparing decoded folder with registry paths
            for (const [id, rec] of Object.entries(srcWorkspaces)) {
              if (!rec || !rec.path) continue;
              
              // Normalize both paths for comparison
              const normalizedRecPath = rec.path.replace(/[\/\\]/g, '/').toLowerCase();
              const normalizedDecodedFolder = decodedFolder.replace(/[\/\\]/g, '/').toLowerCase();
              
              // Check if paths match (either exact match or one contains the other)
              if (normalizedRecPath === normalizedDecodedFolder || 
                  normalizedRecPath.includes(normalizedDecodedFolder) ||
                  normalizedDecodedFolder.includes(normalizedRecPath)) {
                // Found matching workspace, check if there's a mapping
