...
function apply(ctx) {
  ...
  function fixWindowsZipStructure(dir) {
    ...
    // Reorganize files with backslash paths
    for (const entry of entries) {
      if (!entry.includes('\\')) continue;
      
      const oldPath = join(dir, entry);
      // Convert backslashes to forward slashes and split into path segments
      ...
      try {
        ...
        if (st.isFile()) {
          ...
          // It's a directory (empty dir marker), just remove it
