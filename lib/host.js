    ...
    try {
      ...
      if (existsSync(sessionsDir)) {
        ...
        const importSessionDir = (sessPath, id, rawWorkspace) => {
          ...
          // 同 id 在其他本机工作区已有落位 → 按"移动"处理：先删旧位置（DSH 按
          // 会话 id 全局索引，两份同 id 拷贝会让头部解析选错、attach 被拒），
          // 并丢弃其投影缓存文档，让标题按新身份重建。会话正被打开时不移动。
          ...
          if (otherHomes.length > 0) {
            ...
            if (live !== undefined) {
              ...
              for (const h of otherHomes) {
                try { rmSync(h.dir, { recursive: true, force: true }); removedAny = true; } catch (e) { /* skip */ }
              }
              if (removedAny) {
                for (const h of otherHomes) movedFromFolders.push(h.folder);
