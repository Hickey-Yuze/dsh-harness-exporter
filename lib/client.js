		...
		function ExportImportSettingsSection() {
			...
			const browseExportDir = async () => {
				...
				if (typeof pick !== "function") {
					// No native picker available, use default path
					await applyDefaultPath();
					return;
				}
				try {
			...
			const doInspect = async (file) => {
				...
				try {
					...
					const data = await res.json();
					if (data.error) throw new Error(data.error);
					setInspectedContents({ configs: data.configs, presets: data.presets, sessions: data.sessions });
